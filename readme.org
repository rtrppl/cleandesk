...
=cleandesk-show-tree= combines the output of =tree= and =fd= to create a tree of folders for the current folder shown in Dired. Every folder name is linked to that folder (see screenshot above). Use n, p to navigate, o to open in Dired, O to open using system default (this uses =org-open-at-point=), and q for exit. 
